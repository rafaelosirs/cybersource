<?php
	...
	class CI_CyberSource {
		
		const ENV_TEST = 'https://ics2wstest.ic3.com/commerce/1.x/transactionProcessor/CyberSourceTransaction_1.148.wsdl';
		const ENV_PRODUCTION = 'https://ics2ws.ic3.com/commerce/1.x/transactionProcessor/CyberSourceTransaction_1.67.wsdl';
		
		// para saber como se reciben todos los campos del wsdl de cybersource ve a https://ics2wsa.ic3.com/commerce/1.x/transactionProcessor/CyberSourceTransaction_1.148.xsd
		
		const VERSION = '0.3';
		const API_VERSION = '1.148';
		...
		public function device_fingerprint ( $device ){
			$this->device_fingerprint = $device;
			return $this;
		}
		...
		public function merchant_defined_data4 ( $data ) {
		
			$this->merchant_defined_data4 = $data;
		...
		public function add_item ( $price, $quantity = 1, $sku, $name, $additional_fields = array()) {
		...
		private function create_mdd_to ( ) {
			
			$merchantDefinedData = new \stdClass();
			$merchantDefinedData->field1 = 'DIGITALGOODS';
			$merchantDefinedData->field2 = "Punta_Cana_Trading";
			$merchantDefinedData->field3 = "Web";
			$merchantDefinedData->field4 = $this->merchant_defined_data4;			
		...
		public function authorize ( $amount = null ) {
			...
			// add billing info to the request
			$request->merchantDefinedData = $this->create_mdd_to();
		...
		public function charge ( $amount = null ) {
			...
			// add billing info to the request
			$request->MDD = $this->create_mdd_to();
		...
		protected function create_request ( ) {
			...
			$request->deviceFingerprintID = $this->device_fingerprint;
			...
			//$request->merchantDefinedData = array('field1'=>'DIGITALGOODS');
			$item1 = new stdClass();
			$item1->unitPrice = "56.78";
			$item1->id = "1";
			
			
			//$request->item[] = array('unitPrice'=>5, 'productSKU'=>2, 'productName'=>'Curso');
			...
			foreach($this->item AS $it)
				...
				$item0->unitPrice = "12";
				$item0->quantity = "2";
				$item0->id = $it->academy_courses_id;
